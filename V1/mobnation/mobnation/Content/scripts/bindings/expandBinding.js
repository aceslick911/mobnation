...
function _expandActive(elem, selectors, height, width, top, left, duration) {
...
function _toggleExpand(elem, selectors, height, width, top, left, duration) {
    _collapseLast(duration, selectors);

    $(elem).on("click", selectors, function () {
        _collapseLast(duration, selectors);
...
ko.bindingHandlers['expandMore'] = {
    init: function (elem, valueAccessor) {
        ...
        _toggleExpand(elem, (valueAccessor().selectors == null ? ".more-toggle" : valueAccessor().selectors), (valueAccessor().height == null ? 0 : valueAccessor().height), (valueAccessor().width == null ? 0 : valueAccessor().width), (valueAccessor().top == null ? 0 : valueAccessor().top), (valueAccessor().left == null ? 0 : valueAccessor().left), (valueAccessor().duration == null ? 200 : valueAccessor().duration));
    },
